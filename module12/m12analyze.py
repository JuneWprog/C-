            ...
            if icmp_header.code == 3 and icmp_header.type == 3:
                ...
                if IPAddress(ip_header.src_address) in IPNetwork(subnet):
                    ...
                    if raw_buffer[len(raw_buffer)-len(magic_message):] == magic_message:
                        printlist.append("Host=%s   Proto=%s"  % (ip_header.src_address,ip_header.protocol))
...
# to setup promiscuous mode
    ...
    if len (sys.argv)<2:
        print "the format is: sudo python m12analyze.py seconds(int)"
        sys.exit(-1)
    else:    
    #set timer 
...
# Event object used to send signals from one thread to another
    ...
    action_thread = Thread(target=loopthrough)
    action_thread.start()
    ...
    for key in printlist:
        printdic[key]=printdic.get(key,0)+1
    for key in printdic:
        print ("%s count=%s")%(key, printdic[key])
